...
try:
    ...
    if mi48 is None:
        logger.error("Failed to initialize MI48 object. Check your connections and configurations.")
        logger.debug(f"Connected port: {connected_port}, Available ports: {port_names}")
        sys.exit(1)
    logger.info("MI48 initialized successfully.")
except Exception as e:
    logger.error(f"Error during MI48 initialization: {e}")
    sys.exit(1)

# Fetch and log camera info
try:
    logger.info(f"Camera info: {mi48.camera_info}")
except AttributeError as e:
    logger.error("Camera object not properly initialized. 'camera_info' attribute is unavailable.")
    sys.exit(1)
...
# Socket setup for sending frames to the client
server_ip = '172.28.42.196'  # Listening on all available interfaces
server_port = 12345
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.bind((server_ip, server_port))
server_socket.listen(1)  # Only allow 1 client connection
